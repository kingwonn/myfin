...
const FETCH_PROMPT = (source, angle) =>
  ...
  "## Task\n1. Use WebFetch to retrieve the page content. If WebFetch fails (403/429/timeout/blocked), DO NOT give up: fall back to WebSearch — search the exact page title in quotes, the URL, and 2-3 targeted queries about the source's key topic; reconstruct the source's content from search snippets, mirrors and reprints (news is widely syndicated). Extract claims from the reconstructed content and append ' [via search snippets]' inside the quote field. Only return claims: [] if BOTH direct fetch AND search reconstruction yield nothing.\n" +
...
const searchResults = await pipeline(
  ...
  angle => agent(SEARCH_PROMPT(angle), {
    ...
    model: "sonnet",   // model tiering: search execution on Sonnet 5 (Fable plans & synthesizes only)
  }).then(r => {
    if (!r) return null
    ...
    return parallel(
      novel.map(source => () => {
        ...
        return agent(FETCH_PROMPT(source, searchResult.angle), {
          ...
          model: "sonnet",   // model tiering: mechanical fetch+extract on Sonnet 5
        }).then(ext => {
    ...
    parallel(
      Array.from({ length: VOTES_PER_CLAIM }, (_, v) => () =>
        agent(VERIFY_PROMPT(claim, v), {
          ...
          // model tiering: voter 0 on Opus 4.8 (stronger skeptic), voters 1-2 on Sonnet 5
          model: v === 0 ? "opus" : "sonnet",
        })
      )
    ).then(verdicts => {
